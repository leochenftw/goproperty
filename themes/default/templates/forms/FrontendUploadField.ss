<div id="{$form.name.LowerCase}-{$name.LowerCase}-uploader" class="field upload ss-upload ss-uploadfield">
	<% if $canUpload || $canAttachExisting %>
		<div class="ss-uploadfield-item ss-uploadfield-addfile<% if $CustomisedItems %> borderTop<% end_if %>">
			<% if canUpload %>
			<div class="ss-uploadfield-item-preview ss-uploadfield-dropzone docking-bay ui-corner-all<% if not $form.Portrait %> cropper-hold<% end_if %>">
				<% if $form.Portrait %>
                    $form.Portrait
				<% else %>
					<img class="default-avatar" src="/themes/default/images/default-portrait.png" />
				<% end_if %>
			</div>
			<% end_if %>
		...
		<div class="text-centered buttons">
			<a href="#" class="btn-file-browser as-block">Choose image...</a>
		</div>
	<% end_if %>
</div>
